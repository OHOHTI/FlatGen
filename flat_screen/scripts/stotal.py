...
def load_scores(file_pattern):
    # Read the jsonl file (lines=True is required for jsonl)
    ...
    for file in files:
        try:
            ...
            if 'material_id' in df.columns:
                 # Select and Rename
                df = df[['material_id', 'S_bandwidth', 'S_DOS']].rename(columns={'material_id': 'Key'})
                
                # Clean numerics
                df['S_DOS'] = pd.to_numeric(df['S_DOS'], errors='coerce')
                df['S_bandwidth'] = pd.to_numeric(df['S_bandwidth'], errors='coerce')
...
def save_parameters(config, output_file):
...
def main():
    ...
    input_file_pattern = "../data/scores/score_MP_*.jsonl"
    config_file = "../data/stotal_config.json"
    scores_data = load_scores(input_file_pattern)
    print("Loaded data:")
    print(scores_data.head(), scores_data.shape)
    print(
    ...
    save_parameters(config, config_file)
