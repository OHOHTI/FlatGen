...
def calculate_dos_and_scores(bs_info, dos_data: dict, omega_max, alpha=0.2, delta=10.0, mu=2.0) -> tuple:
    flattest_energies = bs_info.get('flattest_curve_energies')
    if flattest_energies is None:
        print("No flattest curve energies available.")
        return None, None
    ...
    dos_up = np.array(dos_data['densities']['1']) if dos_data['densities'].get('1') is not None else None
    dos_down = np.array(dos_data['densities']['-1']) if dos_data['densities'].get('-1') is not None else None
