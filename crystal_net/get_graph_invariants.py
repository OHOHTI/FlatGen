...
from collect_and_output_new import gathered_results
import colorcet as cc
from config import ParamObj
from io_utils import load_structure, parse_systre
...
def get_bs_invariants(gra, keep_list, structure):
    ...
    for hop_ind, hopping in enumerate(this_gra.hoppings):
        if hopping_parameter is None:
            hopping_parameter = hopping['amplitude']
        else:
            if np.abs(hopping_parameter - hopping['amplitude']) > 0.0001:
                uniform_hopping = False
        this_gra.hoppings[hop_ind]['amplitude'] = hopping_parameter
...
def get_invariants(unique_FBs, param, mat_ind, do_systre=False):
    ...
    suffix_list = [f"dmax{max_dist}_decay{decay_rate}" for max_dist, decay_rate in zip(max_dists, decay_rates)]
    ...
    if unique_FBs.p02_no_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[0]}" / mp_id
    elif unique_FBs.p05_no_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[1]}" / mp_id
    elif unique_FBs.p1_no_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[2]}" / mp_id
    elif unique_FBs.p2_no_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[3]}" / mp_id
    elif unique_FBs.p4_no_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[4]}" / mp_id
    elif unique_FBs.p02_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[5]}" / mp_id
    elif unique_FBs.p05_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[6]}" / mp_id
    elif unique_FBs.p1_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[7]}" / mp_id
    elif unique_FBs.p2_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[8]}" / mp_id
    elif unique_FBs.p4_decay[mat_ind]:
        mat_path = Path("crystal_net_results") / f"TB_{suffix_list[9]}" / mp_id
    ...
    found_json_files = list(mat_path.glob("*_results.json"))
    
    if not found_json_files:
        print(f"Warning: No results file found in {mat_path}. Skipping...")
        return [] # Skip this material gracefully

    json_path = found_json_files[0]
    
    # Extract the actual stem to find the corresponding pickle later
    real_file_stem = json_path.name.replace("_results.json", "")
    ...
    pickle_path = mat_path / f"{real_file_stem}_tbmodel.pickle"
    
    if not pickle_path.exists():
         print(f"Warning: Corresponding pickle not found: {pickle_path}")
         return []
    ...
    for comp_ind in range(len(comps_with_FBs_list)):
        ...
        invariants["systre_key"] = str(systre_key)
...
if __name__ == "__main__":
    ...
    if recalc_invariants:
        ...
        with open(f"crystal_net_results/unique_results.pickle", "rb") as f_unique:
        ...
        with open(
            f"crystal_net_results/graph_invariants.pickle", "wb"
        ) as f_pickle:
            pickle.dump({"invariants_list": invariants_list}, f_pickle)
    else:
        # load graph invariants
        with open(
            f"crystal_net_results/graph_invariants.pickle", "rb"
        ) as f:
    ...
    if reclean_invariants:
        ...
        with open(
            f"crystal_net_results/graph_invariants_sorted.pickle", "wb"
        ) as f_pickle:
            pickle.dump(
                {
    ...
    for group_ind in range(30):
        ...
        CifWriter(structure).write_file(
            f"./collision_graph_invariants_sorted_unif/uncategorized/{group_ind}x{counts[sort_inds[group_ind]]}"
